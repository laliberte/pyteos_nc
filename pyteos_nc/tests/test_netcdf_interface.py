...
def test_interface_pottempequi(tmpdir):
    compute_pottemps(tmpdir.strpath, 1, True, False)
    tmpdir_files = [fn.basename for fn in tmpdir.listdir()]
    args = Namespace(in_thermodynamic_file=tmpdir.join('massfraction_air_pottempequi_g_ref_common_pressures.dat').strpath,
                     in_netcdf_file=tmpdir.join('in.nc').strpath,
                     out_netcdf_file=tmpdir.join('out.nc').strpath,
                     zlib=False, memory_percent=0.01)
    ds = xr.Dataset({'ta': (['time', 'plev', 'lat', 'lon'], np.array(300).reshape((1, 1, 1, 1))),
                     'pa': (['time', 'plev', 'lat', 'lon'], np.array(8.5e4).reshape((1, 1, 1, 1))),
                     'hus': (['time', 'plev', 'lat', 'lon'], np.array(1.0 - 0.99).reshape((1, 1, 1, 1)))},
                    coords={'time': [datetime.datetime(2000, 1, 1)],
                            'plev': [8.5e4],
                            'lat': [45.0],
                            'lon': [0.0]})
    ...
    create_thermo(args)
    with Dataset(args.out_netcdf_file) as output:
        np.testing.assert_allclose(output.variables['pottempequi'][:].squeeze(), 341.46261045371557, atol=1.0)
